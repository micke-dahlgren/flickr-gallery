...
export default function SlideShow({ images, handleClose, startAtIndex }) {
	...
	const loadbarStyles = {
		...
		height: '4px',
		width: (countdown * 1000 / (slideShowSpeed - 1000)) * 120 + 'vw',
		maxWidth: '100vw',
	...
	const autoPlay = () => {
		if (images.length - 1 <= imgIndex) {
			...
			const scrollOptions = { inline: "start", block: 'end', behavior: "smooth" }
	...
	useInterval(() => {
		const speedInSec = parseInt(slideShowSpeed / 1000)
	...
	return (
		<>
			...
			<div onClick={handleClose} className="slide-container" >
				<Button variant="secondary" className="close-btn">{spinnerSvg()}</Button>
						...
						images.map((item, idx) => {
							return (
								<div
									...
									id={idx}
									className={parseInt(idx) === imgIndex ? "img-thumb selected" : "img-thumb"}
									key={item.title + idx.toString()}
									onClick={(e) => { e.stopPropagation(); setImgIndex(idx); setCountdown(0); }}
								>
